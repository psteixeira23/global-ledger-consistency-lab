...
from shared.db import AccountORM, Base


@pytest.fixture()
def db_url(tmp_path: Path) -> str:
    return f"sqlite+pysqlite:///{tmp_path / 'payments_api.db'}"


@pytest.fixture(autouse=True)
def clean_environment(db_url: str) -> Iterator[None]:
        ...
        with session.begin():
            ...
            session.add_all(
                [
                    ...
                    AccountORM(id="acc-003", available_balance_cents=1_000, reserved_balance_cents=0, version=0),
                    AccountORM(id="acc-004", available_balance_cents=1_000, reserved_balance_cents=0, version=0),
                ]
            )
    finally:
        session.close()
